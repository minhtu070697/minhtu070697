...
func enter(_msg := {}) -> void:
	...
	if (
		skill.lvl <= 0
		or (
			skill.skill_info.is_target
			and not Utils.node_exists(character.attack_target)
			)
	):
		var find_success: bool = character.target_nearest_enemy(500)
		if not find_success:
			idle()
			return
	...
	character.load_tool_sprite(character.get_tool("casting"), "casting")
	character.show_tool()
	...
	if check_skill_in_range(skill):
		...
		if skill.skill_info.is_target:
			character_stop()
			var direction_to_target = Utils.angle_to_direction(
				character.distance_to_target(character.attack_target.global_position).angle()
			)
		...
		character.stats_manager.attack_library.skill(
			skill, character.attack_target, skill.skill_info.attack_name
		)
