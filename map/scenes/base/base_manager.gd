...
export(int) var map_type = Constants.MapSceneType.BASE_MAP
...
onready var wall_1_tile = TileManager.new(
	$Floor_1/Wall_1_TileMap, TileDescriptions.wall, floor_tile
)
onready var floor_2_tile = TileManager.new($Floor_2/Floor_2_TileMap, TileDescriptions.floors)
onready var wall_2_tile = TileManager.new(
	$Floor_2/Wall_2_TileMap, TileDescriptions.wall, floor_tile
)
...
func _ready():
	...
	base_generator = BaseGenerator.new(
		self, noise_seed, max_size, size, map_position, xrange, yrange
	)
	base_generator.generate_terrain()
	map_navigator = MapNavigatorManager.new(self, max_size, size, map_position)
	base_generator.generate_objects()

	GameManager.save_load_manager.load_current_timing()
	GameManager.audio_manager.play_music(self)
	GameManager.scene_manager.open_scene(GameManager.scene_manager.base_title)
...
func _unhandled_input(event):
	...
	if (
		event.is_action_pressed("go_up_stairs")
		and can_go_up_down_stairs
		and not is_changing_floor
		and cur_floor < 2
	):
		var character = Utils.get_current_character()
		go_up_down(character, true)
	if (
		event.is_action_pressed("go_down_stairs")
		and can_go_up_down_stairs
		and not is_changing_floor
		and cur_floor >= 1
	):
...
func append_floor_object(object):
	match cur_floor:
		Constants.FloorType.GROUND_FLOOR:
			objects_ground_floor.append(object)
		Constants.FloorType.FIRST_FLOOR:
			objects_1st_floor.append(object)
		Constants.FloorType.SECOND_FLOOR:
			objects_2nd_floor.append(object)
...
func pop_floor_object(object):
	match cur_floor:
		Constants.FloorType.GROUND_FLOOR:
			objects_ground_floor.erase(object)
		Constants.FloorType.FIRST_FLOOR:
			objects_1st_floor.erase(object)
		Constants.FloorType.SECOND_FLOOR:
			objects_2nd_floor.erase(object)
...
func is_near_by_stairs():
	...
	return (
		(local_point.x == 0 and (local_point.y == xrange.y - 1 or local_point.y == xrange.y - 2))
...
func go_up_down(character, is_up):
	...
	if is_floor_changable:
		...
		yield(Utils.start_coroutine(.5), "completed")
...
func floor_object_visible():
	...
	match cur_floor:
		Constants.FloorType.GROUND_FLOOR:
			GameManager.scene_manager.change_status_custom(
				GameManager.scene_manager.ground_floor_title, .25, .5
			)
			if objects_ground_floor.size() > 0:
				for i in range(objects_ground_floor.size()):
					...
					Utils.start_tween(
						floor_tween,
						objects_ground_floor[i],
						"modulate",
						objects_ground_floor[i].modulate,
						fade_in_color,
						duration,
						Tween.TRANS_LINEAR,
						Tween.TRANS_LINEAR
		...
		Constants.FloorType.FIRST_FLOOR:
			GameManager.scene_manager.change_status_custom(
				GameManager.scene_manager.first_floor_title, .25, .5
			)
			if objects_1st_floor.size() > 0:
				for i in range(objects_1st_floor.size()):
		...
		Constants.FloorType.SECOND_FLOOR:
			GameManager.scene_manager.change_status_custom(
				GameManager.scene_manager.second_floor_title, .25, .5
			)
			if objects_2nd_floor.size() > 0:
				for i in range(objects_2nd_floor.size()):
...
func go_up_down_character_postion(character, is_in):
	...
	if is_in:
		...
		final_pos = Vector2(
			character.position.x - cell_size.x / 2, character.position.y - cell_size.y / 2
		)
	else:
		begin_pos = Vector2(
			character.position.x - cell_size.x / 2, character.position.y - cell_size.y / 2
...
func spawn_character_from_save_file():
	for i in 1:
		...
		_farmer_character.global_position = GameManager.map_manager_utils.map_to_global_position(
		...
		if _load_save_character:
			...
			_farmer_character.stats_manager.load_new_character_inventory(
